...
function populateEquipsTab() {
	document.getElementById('textEquips1').innerHTML = window['lightHeadArmor'+cow.bestlightHeadArmor].name;
	document.getElementById('textEquips2').innerHTML = window['lightShoulderArmor'+cow.bestlightShoulderArmor].name;
	document.getElementById('textEquips3').innerHTML = window['lightGloves'+cow.bestlightGloves].name;
	document.getElementById('textEquips4').innerHTML = window['lightBodyArmor'+cow.bestlightBodyArmor].name;
	document.getElementById('textEquips5').innerHTML = window['lightPants'+cow.bestlightPants].name;
	...
	document.getElementById('textEquips13').innerHTML = window['mediumBodyArmor'+cow.bestmediumBodyArmor].name;
	document.getElementById('textEquips14').innerHTML = window['mediumPants'+cow.bestmediumPants].name;
	document.getElementById('textEquips15').innerHTML = window['mediumShoes'+cow.bestmediumShoes].name;
	...
	document.getElementById('textEquips22').innerHTML = window['heavyBodyArmor'+cow.bestheavyBodyArmor].name;
	document.getElementById('textEquips23').innerHTML = window['heavyPants'+cow.bestheavyPants].name;
	document.getElementById('textEquips24').innerHTML = window['heavyShoes'+cow.bestheavyShoes].name;
	document.getElementById('textEquips25').innerHTML = window['heavyShield'+cow.bestheavyShield].name;
	document.getElementById('textEquips26').innerHTML = window['mace'+cow.bestmace].name;
	document.getElementById('textEquips27').innerHTML = window['spear'+cow.bestspear].name;
...
function addItemDropToInventory (q) {
	...
	if (q.substring(0,1) == "T") {
		...
		if (r.slice(-1) > cow['best'+r.slice(0, -1)]) {
			...
			document.getElementById('equipsButton').innerHTML = "<span id='IEhack'>NEW!</span>";									// Puts NEW! text on the equips button
...
// Level up skills
	if (cow.axeXP >= 100)		{ cow.axeXP = 0; 		cow.axe++; 		cow.defXP++;	 cow.levelXP++; 	updateConsole("AXE skill is now level " + cow.axe); recalculateWeaponDMG(); recalculatePlayerDMG();}
	if (cow.spearXP >= 100)		{ cow.spearXP = 0;		cow.spear++; 	cow.defXP++;	 cow.levelXP++; 	updateConsole("SPEAR skill is now level " + cow.spear); recalculateWeaponDMG(); recalculatePlayerDMG();}
	if (cow.hArmorXP >= 100)	{ cow.hArmorXP = 0; 	cow.hArmor++; 	cow.defXP++;	 cow.levelXP++; 	updateConsole("H ARMOR skill is now level " + cow.hArmor); recalculateArmorPieceAC(); recalculatePlayerAC();}
	if (cow.endureXP >= 100) 	{ cow.endureXP = 0; 	cow.endure++; 	cow.defXP++;	 cow.levelXP+= .25;	updateConsole("ENDURE skill is now level " + cow.endure)}
	if (cow.swordXP >= 100)		{ cow.swordXP = 0; 		cow.sword++; 	cow.strXP++;	 cow.levelXP++; 	updateConsole("SWORD skill is now level " + cow.sword); recalculateWeaponDMG(); recalculatePlayerDMG();}
	if (cow.maceXP >= 100)		{ cow.maceXP = 0; 		cow.mace++; 	cow.strXP++;	 cow.levelXP++; 	updateConsole("MACE skill is now level " + cow.mace); recalculateWeaponDMG(); recalculatePlayerDMG();}
	if (cow.mArmorXP >= 100)	{ cow.mArmorXP = 0; 	cow.mArmor++; 	cow.strXP++;	 cow.levelXP++; 	updateConsole("M ARMOR skill is now level " + cow.mArmor); recalculateArmorPieceAC(); recalculatePlayerAC();}
	if (cow.smashXP >= 100)		{ cow.smashXP = 0; 		cow.smash++; 	cow.strXP++;	 cow.levelXP+= .25;	updateConsole("SMASH skill is now level " + cow.smash); recalculateWeaponDMG(); recalculatePlayerDMG();}
	if (cow.daggerXP >= 100) 	{ cow.daggerXP = 0; 	cow.dagger++; 	cow.agiXP++;	 cow.levelXP++; 	updateConsole("DAGGER skill is now level " + cow.dagger); recalculateWeaponDMG(); recalculatePlayerDMG();}
	if (cow.dodgeXP >= 100)		{ cow.dodgeXP = 0; 		cow.dodge++; 	cow.agiXP++;	 cow.levelXP++; 	updateConsole("DODGE skill is now level " + cow.dodge)}
	if (cow.lArmorXP >= 100)	{ cow.lArmorXP = 0; 	cow.lArmor++; 	cow.agiXP++;	 cow.levelXP++; 	updateConsole("L ARMOR skill is now level " + cow.lArmor); recalculateArmorPieceAC(); recalculatePlayerAC();}
	if (cow.climbXP >= 100)		{ cow.climbXP = 0; 		cow.climb++; 	cow.agiXP++;	 cow.levelXP+= .25;	updateConsole("CLIMB skill is now level " + cow.climb)}
	if (cow.fistXP >= 100)		{ cow.fistXP = 0; 		cow.fist++; 	cow.spdXP++;	 cow.levelXP++; 	updateConsole("FIST skill is now level " + cow.fist); recalculateWeaponDMG(); recalculatePlayerDMG();}
	if (cow.blockXP >= 100)		{ cow.blockXP = 0; 		cow.block++; 	cow.spdXP++;	 cow.levelXP++; 	updateConsole("BLOCK skill is now level " + cow.block)}
	if (cow.athleteXP >= 100)	{ cow.athleteXP = 0;	cow.athlete++; 	cow.spdXP++;	 cow.levelXP+= .25;	updateConsole("ATHLETE skill is now level " + cow.athlete)}
	if (cow.stealthXP >= 100)	{ cow.stealthXP = 0;	cow.stealth++; 	cow.spdXP++;	 cow.levelXP+= .25;	updateConsole("STEALTH skill is now level " + cow.stealth)}
	if (cow.speechXP >= 100)	{ cow.speechXP = 0; 	cow.speech++;	cow.chaXP++;	 cow.levelXP+= .25;	updateConsole("SPEECH skill is now level " + cow.speech)}
	if (cow.unlockXP >= 100)	{ cow.unlockXP = 0; 	cow.unlock++;	cow.chaXP++;	 cow.levelXP+= .25;	updateConsole("UNLOCK skill is now level " + cow.unlock)}
	if (cow.medicXP >= 100)		{ cow.medicXP = 0; 		cow.medic++; 	cow.chaXP++;	 cow.levelXP+= .25;	updateConsole("MEDIC skill is now level " + cow.medic)}
	if (cow.criticalXP >= 100)	{ cow.criticalXP = 0;	cow.critical++; cow.lukXP++;	 cow.levelXP+= .25;	updateConsole("CRIT skill is now level " + cow.critical)}
...
function updateEXPProgressBars() {
	...
	if (cow.currentImprovingSkill == "invis") {
		...
		document.getElementById('textCurrentlyGrindingSkill').innerHTML = cow.currentImprovingSkill.toUpperCase();
...
function incrementGame() {
	...
	if (cow.minutesPlayed % 5 == 0 && cow.gameplayClock == 1)	{ updateConsole ("Game saved.."); }
